...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART1)
    {
        ...
        // 3. 关键：无条件重启 UART1 接收中断（确保持续接收后续字节）
        // 接收下一个字节，存入缓冲区当前长度对应的位置
        HAL_UART_Receive_IT(&huart1, 
                            (uint8_t*)&strEsp8266_Fram_Record.Data_RX_BUF[strEsp8266_Fram_Record.InfBit.FramLength], 
                            1);

        // 4. 判断帧是否完成（检测到 MQTT 报文结尾 '}'，标记完成）
        if (strchr(strEsp8266_Fram_Record.Data_RX_BUF, '}') != NULL &&
            strEsp8266_Fram_Record.InfBit.FramLength > 10)
        {
            strEsp8266_Fram_Record.InfBit.FramFinishFlag = 1; // 标记帧完成，供主循环解析
        }
    }
}
...
/**
  ...
  * @note   1. 退出核心指令："+++"（无回车、无换行，纯3个加号，避免被当作数据透传）
  *         2. 关键时序：发送前延时确保当前透传数据发送完成，发送后延时让模块处理退出指令
  *         3. 退出后恢复 AT 指令模式，可重新发送 AT 指令配置 TCP 连接/透传等
  *         4. 无需依赖 Cmd 函数，直接发送纯指令，避免透传模式下 Cmd 函数的 AT 格式干扰
  * @param  无
  * @retval 无
  */
void ESP8266_ExitUnvarnishSend(void)
{
    // 定义退出透传的核心指令（纯3个加号，无任何附加字符）
    const char exit_transparent_cmd[] = "+++";
    bool exit_verify_result = false;

    UART2_Debug_Print("===== Start ESP8266 Exit Transparent Mode =====");

    // 步骤1：延时等待当前透传数据发送完成（关键：避免 +++ 被当作普通数据转发）
    // 若当前有未发送完的透传数据，先等待其发送完毕，防止指令混乱
    HAL_Delay(500);
    UART2_Debug_Print("Exit Info: Wait for pending data transmission completed");

    // 步骤2：关闭 UART1 接收中断（可选，避免退出过程中接收数据干扰指令执行）
    HAL_UART_AbortReceive_IT(&huart1);
    HAL_Delay(100);

    // 步骤3：直接发送 "+++" 退出指令（无 AT 前缀、无 \r\n，纯字符发送）
    // 透传模式下无需 AT 指令格式，直接发送 +++ 即可触发退出
    if (HAL_UART_Transmit(&huart1, 
                          (uint8_t*)exit_transparent_cmd, 
                          strlen(exit_transparent_cmd), 
                          UART_TIMEOUT_MS) != HAL_OK)
    {
        UART2_Debug_Print("Exit Error: Send +++ Command Failed");
        // 发送失败，重新开启接收中断，避免影响后续操作
        HAL_UART_Receive_IT(&huart1, (uint8_t*)&strEsp8266_Fram_Record.Data_RX_BUF[0], 1);
        UART2_Debug_Print("===== ESP8266 Exit Transparent Mode Abort =====");
        return;
    }
    UART2_Debug_Print("Exit Info: Send +++ Command Success, Waiting for Module Response");

    // 步骤4：延时等待模块处理退出指令（核心时序，至少 1000ms，确保模块稳定返回 AT 模式）
    // ESP8266 接收到 +++ 后，需要约 1s 时间退出透传，恢复 AT 指令解析能力
    HAL_Delay(1500);

    // 步骤5：重新开启 UART1 接收中断，恢复后续 AT 指令处理能力
    if (HAL_UART_Receive_IT(&huart1, 
                            (uint8_t*)&strEsp8266_Fram_Record.Data_RX_BUF[0], 
                            1) != HAL_OK)
    {
        UART2_Debug_Print("Exit Warn: Re-Enable UART1 Receive Interrupt Failed");
    }

    // 步骤6：验证是否成功退出透传（发送 AT 指令，校验 OK 响应）
    // 退出透传后，模块应恢复 AT 指令解析能力，发送 AT 会返回 OK
    exit_verify_result = ESP8266_Cmd("AT", "OK", NULL, 1000);
    if (exit_verify_result)
    {
        UART2_Debug_Print("Exit Success: ESP8266 Has Exited Transparent Mode (Back to AT Mode)");
    }
    else
    {
        UART2_Debug_Print("Exit Warn: ESP8266 May Not Exit Transparent Mode (Check Timing)");
    }

    UART2_Debug_Print("===== ESP8266 Exit Transparent Mode Process Completed =====");
}
/**
...
uint8_t ESP8266_Get_LinkStatus(void)
{
    ...
    // 步骤2：发送 AT+CIPSTATUS 指令，查询网络状态（校验 OK 响应，超时 1000ms）
    UART2_Debug_Print("LinkStatus Info: Query ESP8266 Network Status (AT+CIPSTATUS)");
    cmd_result = ESP8266_Cmd("AT+CIPSTATUS", "OK", NULL, 1000);
    if (!cmd_result)
    {
    ...
    if (esp_status >= 2)  // 已获取 IP（STATUS=2）或已建立连接（STATUS=3）
    {
        if (conn_status_start != NULL)
        {
            ...
            link_status = 3;
            UART2_Debug_Print("LinkStatus Info: ESP8266 Has Established Connection (Code: 3)");
        }
        else
        {
            ...
            if (esp_status == 2)
            {
                ...
                link_status = 2;
                UART2_Debug_Print("LinkStatus Info: ESP8266 Has Obtained IP (No Connection, Code: 2)");
            }
            else if (strstr(buf, "CLOSED") != NULL)
            {
                // 解析到 CLOSED：连接已断开，返回 4
                link_status = 4;
                UART2_Debug_Print("LinkStatus Info: ESP8266 Connection Disconnected (Code: 4)");
            }
        }
    }
    else
...
{
    char at_cmd[256] = {0};  // 容纳安可信 MQTT 专属指令
    ...
    UART2_Debug_Print("MQTT Info: Configure MQTT User Param: %s", at_cmd);
    cfg_result = ESP8266_Cmd(at_cmd, "OK", "no change", 2000);
    if (!cfg_result)
    {
    ...
    conn_result = ESP8266_Cmd(at_cmd, "MQTT CONNECTED", "OK", MQTT_CONNECT_TIMEOUT);
...
void ESP8266_Aithinker_MQTT_Example(void)
{
    ...
    // 步骤1：先配置 STA 模式，连接 WiFi（复用已有函数）
    if (!ESP8266_Net_Mode_Choose(STA))
    {
        UART2_Debug_Print("MQTT Example: Configure STA Mode Failed");
        return;
    }
    HAL_Delay(1000);

    if (!ESP8266_LINK_AP((char*)ESP8266_STA_SSID, (char*)ESP8266_STA_PWD))
    {
        UART2_Debug_Print("MQTT Example: Link WiFi AP Failed");
        return;
    }
    HAL_Delay(1000);
...
{
    char at_cmd[256] = {0};        // 容纳完整订阅指令
    char subscribe_topic[128] = {0};// 容纳拼接后的完整订阅主题
    ...
    snprintf(at_cmd, sizeof(at_cmd), 
             "AT+MQTTSUB=%d,\"%s\",%d", 
             ID_NO_0,subscribe_topic, MQTT_SUBSCRIBE_QOS);

    // ******** 步骤4：发送订阅指令，校验 OK 响应（安可信固件订阅成功核心标志）********
    sub_result = ESP8266_Cmd(at_cmd, "OK", NULL, MQTT_CONNECT_TIMEOUT);
    if (!sub_result)
    {
...
void ESP8266_Parse_OneNET_Property(char *recv_buf)
{
    ...
    // 直接解析 Alarm、led、light 字段（无需 params）
    char *alarm_key = strstr(recv_buf, "\"Alarm\":");
    if (alarm_key != NULL)
    {
        alarm_key += strlen("\"Alarm\":");
        while (*alarm_key == ' ') alarm_key++;
        temp_data.Alarm = strstr(alarm_key, "true") != NULL;
    }
		else
			 temp_data.Alarm = g_OneNET_Property_Data.Alarm;
		
		
    char *led_key = strstr(recv_buf, "\"led\":");
    if (led_key != NULL)
    {
        led_key += strlen("\"led\":");
        while (*led_key == ' ') led_key++;
        temp_data.led = strstr(led_key, "true") != NULL;
    }
		else
			 temp_data.led = g_OneNET_Property_Data.led;
		
    char *light_key = strstr(recv_buf, "\"light\":");
    if (light_key != NULL)
    {
        light_key += strlen("\"light\":");
        while (*light_key == ' ') light_key++;
        char light_str[4] = {0};
        uint8_t i = 0;
        while ((*light_key != ',' && *light_key != '}') && i < 3)
        {
            if (*light_key >= '0' && *light_key <= '9')
                light_str[i++] = *light_key;
            light_key++;
        }
        temp_data.light = atoi(light_str);
    }
		else
			 temp_data.light = g_OneNET_Property_Data.light;
    // 更新全局变量
    temp_data.is_valid = (alarm_key != NULL || led_key != NULL || light_key != NULL);
    if (temp_data.is_valid)
    {
        temp_data.is_updated = true;
        g_OneNET_Property_Data = temp_data;
        UART2_Debug_Print("Parse Success: Global Var Updated -> Alarm=%d, led=%d, light=%d",
                          temp_data.Alarm, temp_data.led, temp_data.light);
				//ESP8266_Parse_OneNET_Success_Response();
			//ESP8266_AT_MQTT_Publish_Raw(g_OneNET_Property_Data.Alarm,g_OneNET_Property_Data.led,g_OneNET_Property_Data.light);
    }
    else
    {
...
{
    char at_cmd[256] = {0};        // 容纳完整订阅指令
    char subscribe_topic[256] = {0};// 容纳拼接后的完整订阅主题
    bool sub_result = false;
 ...
 */
bool ESP8266_AT_MQTT_Publish_Raw(bool Alarm, bool led, uint8_t light)
{
    // ---------------------- 步骤 1：定义变量，拼接纯 JSON 并计算 size ----------------------
    // 1. 缓冲区定义（JSON 保留所有空格，指令帧足够容纳）
    char mqtt_raw_cmd[256] = {0};    // 第一步的 AT+MQTTPUBRAW 指令
    char json_data[256] = {0};       // 第二步要发送的纯 JSON 数据
    char* onenet_topic = ONENET_POST_TOPIC; 
    ...
    UART2_Debug_Print("Success: AT+MQTTPUBRAW ");
		UART2_Debug_Print("data_result: %s", json_data);
    return true;
}
